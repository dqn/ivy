...
- [ ] Phase 5.1: コマンドエディタ MVP
  - [ ] Tauri プロジェクト scaffold (`editors/ivy-editor/`)
  - [ ] YAML 読み込み・保存（既存パーサー再利用）
  - [ ] コマンドリスト表示（テーブル形式）
  - [ ] 単一コマンド編集フォーム
  - [ ] リアルタイム YAML プレビュー（読み取り専用）
  - [ ] バリデーションエラー表示
- [ ] Phase 5.2: フローチャート表示
  - [ ] 既存 `Flowchart` 型を React Flow で描画
  - [ ] ノードクリックでコマンドにジャンプ
  - [ ] ズーム・パン操作
- [ ] Phase 5.3: プレビュー統合（完全統合方式）
  - [ ] `PreviewState` を Tauri コマンドとして実装
  - [ ] プレビューレンダリングを React コンポーネントに移植
  - [ ] エディタ ↔ プレビューの同期
- [ ] Phase 5.4: ドラッグ＆ドロップ
  - [ ] コマンドリストの順序変更
  - [ ] アセットファイルのドロップでパス自動入力
- [ ] Phase 5.5: アセット管理
  - [ ] アセットブラウザ（ファイルツリー）
  - [ ] 画像・音声プレビュー
  - [ ] 未使用アセット検出
